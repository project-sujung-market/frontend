...
let contents_content = document.querySelector('.contents-content');
...
function open_modal() {
    if(contents_name.value == "" || contents_content.value == "" || contents_address.value == "" || contents_price.value == "") {
        alert('형식에 맞게 입력해주세요');
    }
    else {
        modal.style.display = 'block';
        modal_name.textContent = contents_name.value;
        modal_content.textContent = contents_content.value;
        modal_address.textContent = contents_address.value;
        modal_price.textContent = `${Number(contents_price.value)}원`;
    }
}
